...
func QueryCanTransferBalance(clt *core.SDKClient, accessToken string, req *agent.QueryCanTransferBalanceRequest) ([]agent.CanTransferDetail, error) {
	var resp agent.QueryCanTransferBalanceResponse
	if err := clt.GetAPI("v3.0/cg_transfer/query_can_transfer_balance/", req, &resp, accessToken); err != nil {
		return nil, err
	}
