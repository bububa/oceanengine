...
func FundTransferSeqCreate(clt *core.SDKClient, accessToken string, req *agent.FundTransferSeqCreateRequest) (string, error) {
	var resp agent.FundTransferSeqCreateResponse
	if err := clt.Post("2/agent/fund/transfer_seq/create/", req, &resp, accessToken); err != nil {
		return "", err
	}
