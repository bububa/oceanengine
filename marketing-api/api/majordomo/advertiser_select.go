...
func AdvertiserSelect(clt *core.SDKClient, accessToken string, advertiserID uint64) ([]majordomo.Advertiser, error) {
	req := &majordomo.AdvertiserSelectRequest{
		AdvertiserID: advertiserID,
	}
	var resp majordomo.AdvertiserSelectResponse
	if err := clt.Get("2/majordomo/advertiser/select", req, &resp, accessToken); err != nil {
		return nil, err
	}
